...
public class MultipartTool implements Closeable {
    ...
    public void appendField(final String fieldName, final String data, final String contentType) throws IOException {
        connectionW.append("--").append(boundary).append(CRLF).append("Content-Disposition: form-data; name=\"").append(fieldName).append('"').append(CRLF).append("Content-Type: ").append(contentType).append(CRLF).append(CRLF).append(data).append(CRLF).flush();
    }

    public void appendFile(final String fieldName, final byte[] data, final String contentType, final String fileName) throws IOException {
        appendFile(fieldName, data, 0, data.length, contentType, fileName);
    }

    public void appendFile(final String fieldName, final byte[] data, final int offset, final int len, final String contentType, final String fileName) throws IOException {
        appendFilePreamble(fieldName, contentType, len - offset, fileName);
        connectionOS.write(data, offset, len);
        appendFilePS();
    }

    public void appendFile(final String fieldName, final File file) throws IOException {
        appendFile(fieldName, file, URLConnection.guessContentTypeFromName(file.getName()));
    }

    public void appendFile(final String fieldName, final File file, final String contentType) throws IOException {
     ...
     */
    public void appendFile(final String fieldName, final InputStream data, final String contentType, final long contentLength, final String fileName) throws IOException {
    ...
    public void close() throws IOException {
        connectionW.append(CRLF).append("--").append(boundary).append("--").append(CRLF).close();
    }
    ...
    private void appendFilePreamble(String fieldName, String contentType, long contentLength, String fileName) throws IOException {
        connectionW.append("--").append(boundary).append(CRLF).append("Content-Disposition: form-data; name=\"").append(fieldName).append("\"; filename=\"").append(fileName).append('"').append(CRLF).append("Content-Type: ").append(contentType).append(CRLF).append("Content-Length: ").append(Long.toString(contentLength)).append(CRLF).append("Content-Transfer-Encoding: binary").append(CRLF).append(CRLF).flush();
    }
